...
/**
 * Last-resort boundary for the /macro route -- per-endpoint errors are
 * already caught inside each Section component (RatesSection etc.) and
 * rendered as inline ErrorCards, so this only fires for something that
 * escapes that (a render bug, not a data-fetch failure).
 */
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-xl flex-col items-center justify-center gap-4 p-8">
      <div className="w-full rounded-lg border border-red-800 bg-red-950/50 p-4 text-sm">
        <span className="font-medium text-red-400">Failed to load MACRO page</span>
