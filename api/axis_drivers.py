...
DRIVERS: dict[str, list[tuple]] = {
    ...
    "liquidity": [
        # Target ceiling (DFEDTARU, 2008-12->) by the user's choice (2026-09-18):
        # DFF (effective, 1954->) scores the tightening state on 46 windows
        # with identical hike-side terciles, but sits ~12bp under the ceiling
        # and posts a day late, so the "now" reading doesn't match how the
        # user thinks about the spread. DFF stays onboarded for the sweep.
